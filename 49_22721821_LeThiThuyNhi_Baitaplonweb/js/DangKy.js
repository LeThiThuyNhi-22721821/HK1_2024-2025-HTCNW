...
// sửa lại theo chat
function dangky(){
    if(ktraten() && ktraNgaySinh() && ktraEmail() && ktrasodienthoai() && xacNhanMatKhau()){
        var hoten = $('#txthoten').val().trim();
        var sdt = $('#txtsdt').val().trim();
        var email = $('#txtemail').val().trim();
        var ngaysinh = new Date($('#txtngaysinh').val()); // Lấy ngày sinh

        var stt = $('#tableBody tr').length + 1;
        var newRow = $('<tr><td>' + stt + '</td><td>' + hoten + '</td><td>' + ngaysinh.toLocaleDateString() + '</td><td>' + sdt + '</td><td>' + email + '</td></tr>');
